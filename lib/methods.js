...
module.exports = [{
    ...
    method: function(request) {
        var returnValue = this.defaultRights.anonymous || {};
        if (request.auth.isAuthenticated === true) {
            returnValue = request.auth.credentials.userAccessRights || returnValue;
        }

        request.server.log(['clapper', 'getUserAccessRights'], returnValue);
        return returnValue;
    }
}, {
